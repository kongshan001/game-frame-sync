"""
确定性物理引擎

本模块提供帧同步所需的确定性物理模拟：
- Entity: 游戏实体，使用定点数坐标
- PhysicsEngine: 物理引擎，处理碰撞和物理模拟
- EntityPool: 实体对象池，优化内存分配

重构说明：
- 使用 core.fixed.FixedPoint 进行定点数运算
- 使用 core.config.CONFIG 读取配置参数
- 消除所有硬编码的 << 16 和魔法数字
"""

from dataclasses import dataclass, field
from typing import Dict, List, Optional, Tuple, Union
import math

from .fixed import fixed, FixedPoint
from .config import CONFIG


@dataclass
class Entity:
    """
    游戏实体
    
    表示游戏中的一个实体（角色、子弹、道具等）。
    所有坐标和速度使用定点数表示，保证跨平台确定性。
    
    重构说明：
    - 使用 FixedPoint.FRACTION_BITS 替代硬编码的 16
    - 使用 FixedPoint.SCALE 替代硬编码的 65536
    
    属性:
        entity_id (int): 实体的唯一标识符
        
        x (int): X 坐标（定点数原始值）
        y (int): Y 坐标（定点数原始值）
        vx (int): X 方向速度（定点数原始值）
        vy (int): Y 方向速度（定点数原始值）
        width (int): 实体宽度（定点数原始值）
        height (int): 实体高度（定点数原始值）
        hp (int): 当前生命值
        max_hp (int): 最大生命值
        flags (int): 状态标志位
    """
    entity_id: int
    x: int = 0
    y: int = 0
    vx: int = 0
    vy: int = 0
    width: int = 0  # 默认值在 __post_init__ 中设置
    height: int = 0
    hp: int = 0
    max_hp: int = 0
    flags: int = 0
    
    def __post_init__(self):
        """初始化默认值（使用配置）"""
        if self.width == 0:
            self.width = int(CONFIG.physics.ENTITY_WIDTH * FixedPoint.SCALE)
        if self.height == 0:
            self.height = int(CONFIG.physics.ENTITY_HEIGHT * FixedPoint.SCALE)
        if self.hp == 0:
            self.hp = CONFIG.game.DEFAULT_HP
        if self.max_hp == 0:
            self.max_hp = CONFIG.game.DEFAULT_HP
    
    @classmethod
    def from_float(cls, entity_id: int, x: float, y: float) -> 'Entity':
        """
        从浮点数坐标创建实体
        
        Args:
            entity_id: 实体ID
            x: X 坐标（浮点数，像素）
            y: Y 坐标（浮点数，像素）
        
        Returns:
            Entity 实例
        """
        return cls(
            entity_id=entity_id,
            x=int(x * FixedPoint.SCALE),
            y=int(y * FixedPoint.SCALE)
        )
    
    @classmethod
    def from_fixed(cls, entity_id: int, x: FixedPoint, y: FixedPoint) -> 'Entity':
        """
        从 FixedPoint 创建实体
        
        Args:
            entity_id: 实体ID
            x: X 坐标（FixedPoint）
            y: Y 坐标（FixedPoint）
        
        Returns:
            Entity 实例
        """
        return cls(entity_id=entity_id, x=x.raw, y=y.raw)
    
    def to_float(self) -> Tuple[float, float]:
        """
        转换为浮点数坐标
        
        Returns:
            (x, y) 元组，浮点数像素坐标
        """
        return (self.x / FixedPoint.SCALE, self.y / FixedPoint.SCALE)
    
    def to_fixed(self) -> Tuple[FixedPoint, FixedPoint]:
        """
        转换为 FixedPoint 坐标
        
        Returns:
            (x, y) 元组，FixedPoint 坐标
        """
        return (FixedPoint.from_raw(self.x), FixedPoint.from_raw(self.y))
    
    def to_int(self) -> Tuple[int, int]:
        """
        转换为整数像素坐标
        
        Returns:
            (x, y) 元组，整数像素坐标
        """
        return (self.x >> FixedPoint.FRACTION_BITS, self.y >> FixedPoint.FRACTION_BITS)
    
    def update_position(self, dt_ms: int):
        """
        根据速度更新位置
        
        Args:
            dt_ms: 时间增量（毫秒）
        """
        if dt_ms <= 0:
            return
        self.x += (self.vx * dt_ms) // 1000
        self.y += (self.vy * dt_ms) // 1000
    
    def set_velocity(self, vx: Union[float, FixedPoint], vy: Union[float, FixedPoint]):
        """
        设置速度
        
        Args:
            vx: X 方向速度（像素/秒 或 FixedPoint）
            vy: Y 方向速度（像素/秒 或 FixedPoint）
        """
        if isinstance(vx, FixedPoint):
            self.vx = vx.raw
        else:
            self.vx = int(vx * FixedPoint.SCALE)
        
        if isinstance(vy, FixedPoint):
            self.vy = vy.raw
        else:
            self.vy = int(vy * FixedPoint.SCALE)
    
    def get_bounds(self) -> Tuple[int, int, int, int]:
        """
        获取碰撞边界
        
        Returns:
            (x1, y1, x2, y2) 元组，定点数坐标
        """
        return (self.x, self.y, self.x + self.width, self.y + self.height)
    
    def reset(self):
        """重置实体状态（用于对象池）"""
        self.x = 0
        self.y = 0
        self.vx = 0
        self.vy = 0
        self.hp = self.max_hp
        self.flags = 0
    
    def serialize(self) -> dict:
        """序列化为字典"""
        return {
            'id': self.entity_id,
            'x': self.x,
            'y': self.y,
            'vx': self.vx,
            'vy': self.vy,
            'hp': self.hp,
            'flags': self.flags
        }
    
    @classmethod
    def deserialize(cls, data: dict) -> 'Entity':
        """从字典反序列化"""
        return cls(
            entity_id=data['id'],
            x=data['x'],
            y=data['y'],
            vx=data.get('vx', 0),
            vy=data.get('vy', 0),
            hp=data.get('hp', CONFIG.game.DEFAULT_HP),
            max_hp=CONFIG.game.DEFAULT_HP,
            flags=data.get('flags', 0)
        )


class PhysicsEngine:
    """
    确定性物理引擎
    
    处理游戏中所有实体的物理模拟：
    - 重力
    - 摩擦力
    - 碰撞检测和响应
    - 边界处理
    
    重构说明：
    - 所有常量从 CONFIG 读取
    - 使用 FixedPoint 进行计算
    
    属性:
        entities: 所有实体的字典
        collision_pairs: 当前帧检测到的碰撞对
        spatial_grid: 空间划分网格
    """
    
    def __init__(self, config=None):
        """
        初始化物理引擎
        
        Args:
            config: 可选的配置对象（用于测试）
        """
        self._cfg = config or CONFIG
        
        # 从配置读取常量（转换为定点数）
        self.GRAVITY = int(self._cfg.physics.GRAVITY * FixedPoint.SCALE)
        self.FRICTION = int(self._cfg.physics.FRICTION * FixedPoint.SCALE)
        self.MAX_VELOCITY = int(self._cfg.physics.MAX_VELOCITY * FixedPoint.SCALE)
        self.WORLD_WIDTH = int(self._cfg.physics.WORLD_WIDTH * FixedPoint.SCALE)
        self.WORLD_HEIGHT = int(self._cfg.physics.WORLD_HEIGHT * FixedPoint.SCALE)
        self.cell_size = int(self._cfg.physics.GRID_CELL_SIZE * FixedPoint.SCALE)
        
        self.entities: Dict[int, Entity] = {}
        self.collision_pairs: List[Tuple[int, int]] = []
        self.spatial_grid: Dict[Tuple[int, int], List[int]] = {}
    
    def add_entity(self, entity: Entity):
        """添加实体"""
        self.entities[entity.entity_id] = entity
    
    def remove_entity(self, entity_id: int):
        """移除实体"""
        if entity_id in self.entities:
            del self.entities[entity_id]
    
    def get_entity(self, entity_id: int) -> Optional[Entity]:
        """获取实体"""
        return self.entities.get(entity_id)
    
    def update(self, dt_ms: int):
        """
        更新物理模拟
        
        Args:
            dt_ms: 时间增量（毫秒）
        """
        if dt_ms <= 0:
            return
        
        for entity in self.entities.values():
            # 应用重力
            entity.vy += (self.GRAVITY * dt_ms) // 1000
            
            # 限制最大速度
            entity.vx = max(-self.MAX_VELOCITY, min(self.MAX_VELOCITY, entity.vx))
            entity.vy = max(-self.MAX_VELOCITY, min(self.MAX_VELOCITY, entity.vy))
            
            # 更新位置
            entity.update_position(dt_ms)
            
            # 应用摩擦力
            entity.vx = (entity.vx * self.FRICTION) >> FixedPoint.FRACTION_BITS
        
        # 边界碰撞
        self._handle_boundary_collision()
        
        # 实体间碰撞
        self._handle_entity_collision_optimized()
    
    def _handle_boundary_collision(self):
        """处理边界碰撞"""
        for entity in self.entities.values():
            if entity.x < 0:
                entity.x = 0
                entity.vx = 0
            
            if entity.x + entity.width > self.WORLD_WIDTH:
                entity.x = self.WORLD_WIDTH - entity.width
                entity.vx = 0
            
            if entity.y < 0:
                entity.y = 0
                entity.vy = 0
            
            if entity.y + entity.height > self.WORLD_HEIGHT:
                entity.y = self.WORLD_HEIGHT - entity.height
                entity.vy = 0
    
    def _update_spatial_grid(self):
        """更新空间网格"""
        self.spatial_grid.clear()
        for eid, entity in self.entities.items():
            cx = (entity.x + entity.width // 2) // self.cell_size
            cy = (entity.y + entity.height // 2) // self.cell_size
            cell = (cx, cy)
            if cell not in self.spatial_grid:
                self.spatial_grid[cell] = []
            self.spatial_grid[cell].append(eid)
    
    def _handle_entity_collision_optimized(self):
        """优化的实体间碰撞检测"""
        self.collision_pairs.clear()
        self._update_spatial_grid()
        
        checked = set()
        
        for cell, entity_ids in self.spatial_grid.items():
            for i, eid1 in enumerate(entity_ids):
                for eid2 in entity_ids[i+1:]:
                    pair = (min(eid1, eid2), max(eid1, eid2))
                    if pair in checked:
                        continue
                    checked.add(pair)
                    
                    e1 = self.entities[eid1]
                    e2 = self.entities[eid2]
                    if self._check_aabb_collision(e1, e2):
                        self.collision_pairs.append(pair)
                        self._resolve_collision(e1, e2)
            
            # 检查相邻单元格
            cx, cy = cell
            neighbors = [(-1,0), (0,-1), (-1,-1), (1,-1)]
            
            for dx, dy in neighbors:
                neighbor = (cx + dx, cy + dy)
                if neighbor not in self.spatial_grid:
                    continue
                    
                for eid1 in entity_ids:
                    for eid2 in self.spatial_grid[neighbor]:
                        pair = (min(eid1, eid2), max(eid1, eid2))
                        if pair in checked:
                            continue
                        checked.add(pair)
                        
                        e1 = self.entities[eid1]
                        e2 = self.entities[eid2]
                        if self._check_aabb_collision(e1, e2):
                            self.collision_pairs.append(pair)
                            self._resolve_collision(e1, e2)
    
    def _check_aabb_collision(self, a: Entity, b: Entity) -> bool:
        """AABB 碰撞检测"""
        return (a.x < b.x + b.width and
                a.x + a.width > b.x and
                a.y < b.y + b.height and
                a.y + a.height > b.y)
    
    def _resolve_collision(self, a: Entity, b: Entity):
        """解决碰撞"""
        overlap_x = min(a.x + a.width - b.x, b.x + b.width - a.x)
        overlap_y = min(a.y + a.height - b.y, b.y + b.height - a.y)
        
        if overlap_x < overlap_y:
            if a.x < b.x:
                a.x -= overlap_x // 2
                b.x += overlap_x // 2
            else:
                a.x += overlap_x // 2
                b.x -= overlap_x // 2
            a.vx = 0
            b.vx = 0
        else:
            if a.y < b.y:
                a.y -= overlap_y // 2
                b.y += overlap_y // 2
            else:
                a.y += overlap_y // 2
                b.y -= overlap_y // 2
            a.vy = 0
            b.vy = 0
    
    def apply_input(self, entity_id: int, input_flags: int, speed: int = None):
        """
        应用玩家输入到实体
        
        Args:
            entity_id: 实体ID
            input_flags: 输入标志位
            speed: 移动速度（定点数），默认从配置读取
        """
        entity = self.get_entity(entity_id)
        if not entity:
            return
        
        # 从配置读取默认速度
        if speed is None:
            speed = int(self._cfg.game.PLAYER_SPEED * FixedPoint.SCALE)
        
        from .input import InputFlags
        
        vx = 0
        vy = 0
        
        if input_flags & InputFlags.MOVE_LEFT:
            vx -= speed
        if input_flags & InputFlags.MOVE_RIGHT:
            vx += speed
        if input_flags & InputFlags.MOVE_UP:
            vy -= speed
        if input_flags & InputFlags.MOVE_DOWN:
            vy += speed
        
        entity.vx = vx
        entity.vy = vy
    
    def serialize_state(self) -> dict:
        """序列化当前物理状态"""
        return {
            'entities': {
                eid: entity.serialize() 
                for eid, entity in self.entities.items()
            },
            'collisions': self.collision_pairs.copy()
        }
    
    def deserialize_state(self, state: dict):
        """反序列化物理状态"""
        self.entities.clear()
        for eid, data in state.get('entities', {}).items():
            self.entities[int(eid)] = Entity.deserialize(data)


def distance_squared(a: Entity, b: Entity) -> int:
    """计算两实体间距离的平方"""
    dx = a.x - b.x
    dy = a.y - b.y
    return (dx * dx + dy * dy) >> FixedPoint.FRACTION_BITS


def distance(a: Entity, b: Entity) -> int:
    """计算两实体间距离"""
    dist_sq = distance_squared(a, b)
    return isqrt(dist_sq)


def isqrt(n: int) -> int:
    """整数平方根（牛顿法）"""
    if n < 0:
        return 0
    if n == 0:
        return 0
    
    x = n
    y = (x + 1) // 2
    while y < x:
        x = y
        y = (x + n // x) // 2
    
    return x


class EntityPool:
    """
    实体对象池
    
    复用 Entity 对象以减少内存分配。
    """
    
    def __init__(self, initial_size: int = 100):
        """初始化对象池"""
        self._pool: List[Entity] = []
        self._active: set = set()
        
        for i in range(initial_size):
            entity = Entity(entity_id=i)
            self._pool.append(entity)
    
    def acquire(self, entity_id: int) -> Entity:
        """从池中获取实体"""
        if entity_id in self._active:
            raise ValueError(f"Entity {entity_id} already active")
        
        if self._pool:
            entity = self._pool.pop()
            entity.entity_id = entity_id
            entity.reset()
            self._active.add(entity_id)
            return entity
        
        entity = Entity(entity_id=entity_id)
        self._active.add(entity_id)
        return entity
    
    def release(self, entity: Entity):
        """将实体归还到池中"""
        if entity.entity_id in self._active:
            self._active.remove(entity.entity_id)
            self._pool.append(entity)
